...
import { proxyKey } from './proxy_config.js';
...
try {
      ...
      if (request.method === "POST") {
        const { url: bodyUrl, cache } = await request.json();
        url = bodyUrl;
        useCache = (typeof cache === 'string' && cache.toLowerCase() === "off") ? false : true;
      } else {
      ...
      console.log("Using proxy ISP:", proxyKey);
      
      // Check cache for existing response if caching is enabled
      let cacheKey;
      if (useCache) {
          cacheKey = new Request(url, { headers: request.headers });
